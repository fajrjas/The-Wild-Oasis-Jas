 ...
 const activeFilter = searchParams.get('capacity') ?? 'all';
  ...
  return (
    <div className="border border-primary-800 flex">
      ...
      <Button filter={'medium'} handleFilter={handleFilter} activeFilter={activeFilter}>4&mdash;8 guests</Button>
...
function Button({filter,handleFilter,children,activeFilter}){
  return <button className={`px-5 py-2 hover:bg-primary-700 ${activeFilter === filter ? 'bg-primary-700 text-primary-50': ''} `} onClick={()=>handleFilter(filter)}>{children}</button>
}
